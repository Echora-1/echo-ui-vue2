...
export default {
  ...
  methods: {
    ...
    renderHead(h, columnsOptions) {
      ...
      return columnsOptions.map((column) => {
        ...
        return (
          <th
            key={column.prop}
            class={cls.eTable__Th}
            onClick={() => this.sortColum(column.prop)}
          >
            {renderedTitle}{" "}
            {column.sortable && (
              <button class={this.sortBtnClass(column.prop)}>
                {this.sortKey.name === column.prop && !this.sortKey.reverse
                  ? "▼"
                  : "▲"}
              </button>
            )}
    ...
    sortBtnClass(name) {
      return this.sortKey.name === name
        ? cls.eTable__ThSortBtnActive
        : cls.eTable__ThSortBtn;
    },
        ...
        if (this.windowWidthIsMobile) {
          return (
            <td key={column.prop} class={cls.eTable__Td}>
              <div style="font-weight: bold;">{column.prop}</div>
              <div>{body}</div>
            </td>
          );
        }
    ...
    renderRows(h, columnsOptions) {
      return this.rows.map((row, index) => {
        return (
          <tr key={row.id || index} class={cls.eTable__Tr}>
      ...
      return this.$slots.default
        .filter(
          (item) =>
            ...
            item.componentOptions.tag === "e-table-column"
        )
        .map((column) =>
