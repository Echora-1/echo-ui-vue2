...
export default {
  ...
  created() {
    ...
    this.renderedTab.push(this.options.tabItem.find(t => t.index === this.value))
  },
  ...
  methods: {
    getTabsOptions() {
      let tabIndex = 0;
      let tabItemIndex = 0;
      return this.$slots.default.reduce((options, node) => {
        const nodeTag = node.componentOptions.tag
        if(nodeTag === 'e-tab') {
          options.tab.push({children: node.componentOptions.children[0], index: tabIndex})
          tabIndex++
        }
        if(nodeTag === 'e-tab-item') {
          options.tabItem.push({children: node.componentOptions.children[0], index: tabItemIndex})
          tabItemIndex++
        }
        return options
      }, {tab: [], tabItem: []})
    ...
    renderTabItem(tabItems) {
      return tabItems.map(tabItem => (<div key={tabItem.index} class={this.tabItemClass(tabItem.index)}>{tabItem.children}</div>))
    },
  ...
  render(){
    ...
    return (
        <div>
          <div class={cls.tabWrapper}>
            {...tab}
          </div>
          <div class={cls.tabItemWrapper}>
            {...tabItem}
          </div>
        </div>
    )
